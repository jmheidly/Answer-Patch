...
type PostStoreServices interface {
	...
	FindQuestionsByFilter(string, string) []*models.Question
	SortQuestions(string, string, string, string) []*models.Question
...
func (store *PostStore) FindPostByID(id string) (*models.Question, *models.Answer) {
	...
	row, err := store.DB.Query(`SELECT q.id, q.user_id, u.username, q.title, q.content, q.upvotes, q.edit_count, q.category_name, q.submitted_at FROM question q INNER JOIN ap_user u ON q.user_id = u.id WHERE q.id =$1`, id)
	if err != nil {
		log.Fatal(err)
	...
	err = row.Scan(&question.ID, &question.AuthorID, &question.Author, &question.Title, &question.Content, &question.Upvotes, &question.EditCount, &question.Category, &question.SubmittedAt)
	if err != nil {
		log.Fatal(err)
	}
...
func (store *PostStore) FindQuestionsByFilter(filter, val string) []*models.Question {

	queryStmt := `SELECT q.id, q.user_id, u.username, q.title, q.content, q.upvotes, q.edit_count, q.category_name, q.submitted_at FROM question q`
	...
	case filter == "category":
		queryStmt += ` INNER JOIN ap_user u ON q.user_id = u.id WHERE q.category_name = $1`
	}
...
func (store *PostStore) SortQuestions(postComponent, filter, order, offset string) []*models.Question {
...
func (store *PostStore) StoreQuestion(user_id, title, content, category string) {

	transact(store.DB, func(tx *sql.Tx) error {
		_, err := tx.Exec(`INSERT INTO question(user_id, title, content, category_name) values($1::uuid, $2, $3, $4)`, user_id, title, content, category)
		return err
	})
...
func scanQuestions(rows *sql.Rows) []*models.Question {
	...
	for rows.Next() {
		...
		err := rows.Scan(&tempQuestion.ID, &tempQuestion.AuthorID, &tempQuestion.Author, &tempQuestion.Title, &tempQuestion.Content, &tempQuestion.Upvotes, &tempQuestion.EditCount, &tempQuestion.Category, &tempQuestion.SubmittedAt)
		if err != nil {
			log.Fatal(err)
		}
