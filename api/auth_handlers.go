...
func ServeSubmitQuestion(store datastore.PostStoreServices) middleware.ContextRequiredHandlerFunc {
	return func(c *models.Context, w http.ResponseWriter, r *http.Request) {
		...
		if !isUnique {
			...
			store.StoreQuestion(newQuestion.AuthorID, newQuestion.Title, newQuestion.Content)
